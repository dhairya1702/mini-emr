...
async def create_referral_package_workflow(
    ...
    for note_id in note_ids:
        ...
        if str(note.get("status") or "") not in {"final", "sent"}:
            raise ValueError("Only finalized consultations can be included in a referral package.")
        notes.append(note)
    ...
    snapshot = _json_value({
        ...
        "patient": {
            key: patient.get(key) for key in (
                "id", "name", "phone", "email", "address", "date_of_birth", "age", "sex_at_birth", "gender_identity"
            )
        },
    ...
    pdf_bytes, page_count = build_referral_package_pdf(snapshot, attachment_files)
    ...
    await write_audit_event(
        ...
        action="referral_package_created", summary=f"Created referral package for {patient.get('name') or 'patient'}.",
...
async def download_referral_package_workflow(
    ...
    if hashlib.sha256(raw_bytes).hexdigest() != str(row["file_sha256"]):
        raise RuntimeError("Stored referral package failed its integrity check.")
...
async def send_referral_package_workflow(
    ...
    default_message = (
        f"Please find the referral package for {patient.get('name') or 'the patient'} attached. "
        f"Contact {clinic_name} if any further information is required."
    )
            ...
            try:
                if channel == "email":
                    await send_clinic_email_message(
                        ...
                        subject=f"Referral package: {patient.get('name') or 'Patient'}",
    ...
    await write_audit_event(
        ...
        summary=f"Referral package delivery completed with {len(successful)} of {len(delivery_rows)} successful attempts.",
    ...
    return ReferralPackageSendResponse(
        ...
        message=f"Referral package sent to {len(successful)} of {len(delivery_rows)} destinations.",
