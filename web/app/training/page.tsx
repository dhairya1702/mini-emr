  ...
  return (
    <main className="clinic-page">
      <div className="clinic-container">
        ...
        <section className="rounded-[18px] border border-[#dbe7ef] bg-white p-6 shadow-[0_14px_38px_rgba(64,131,181,0.09)]">
          <h1 className="text-2xl font-semibold text-slate-900">Training Mode</h1>
          <p className="mt-3 max-w-3xl text-sm leading-7 text-slate-600">
            Practice queue and consultation flows with sandbox data before using the live clinic workspace.
          </p>
          <div className="mt-5 flex flex-wrap gap-3">
            {isTrainingMode ? (
              <>
                <button type="button" onClick={exitTrainingMode} className="rounded-xl bg-[#2f8fd3] px-5 py-2.5 text-sm font-medium text-white transition hover:bg-[#287fc0]">Exit Training Mode</button>
                <button type="button" onClick={resetTrainingMode} className="rounded-xl border border-[#9fc7e1] bg-white px-5 py-2.5 text-sm font-medium text-slate-800 transition hover:bg-[#f3f8fb]">Reset Sandbox</button>
              </>
            ) : (
              <button type="button" onClick={enterTrainingMode} className="rounded-xl bg-[#2f8fd3] px-5 py-2.5 text-sm font-medium text-white transition hover:bg-[#287fc0]">Enter Training Mode</button>
            )}
          </div>
        </section>
      </div>
      ...
      {isSettingsOpen ? (
        <LazySettingsDrawer
          ...
          onEnterTrainingMode={enterTrainingMode}
          onExitTrainingMode={exitTrainingMode}
          onResetTrainingMode={resetTrainingMode}
        />
      ) : null}
    </main>
  );
