...
function Page({ title, children }: { title: string; children: ReactNode }) {
  return <div className="w-full bg-white text-black"><h3 className="mb-5 text-center text-lg font-bold uppercase underline decoration-1 underline-offset-4">{title}</h3><div className="space-y-6">{children}</div></div>;
}
...
function OcularDrawingSection({ segment, data, onOpen }: { segment: OcularDrawingTarget["segment"]; data: Data; onOpen: (target: OcularDrawingTarget) => void }) {
  return <section className="space-y-3"><h4 className="text-center text-sm font-bold uppercase tracking-[0.08em] text-black">{segment === "anterior" ? "Anterior Segment Drawings" : "Posterior Segment Drawings"}</h4><div className="grid gap-4 md:grid-cols-2">{EYES.map(([eye, label]) => { const drawing = String(get(data, ["examination", "drawings", segment, eye])); return <div key={eye} className="border border-black bg-slate-50 p-3"><div className="mb-3 flex items-center justify-between gap-3"><span className="text-sm font-semibold text-black">{label}</span><button type="button" onClick={() => onOpen({ segment, eye })} className="border border-slate-900 bg-white px-3 py-1.5 text-sm font-semibold text-black hover:bg-slate-100">{drawing ? "Edit drawing" : "Open drawing"}</button></div>{drawing ? <div role="img" aria-label={`${label} ${segment} segment drawing`} className="aspect-[21/10] w-full border border-slate-300 bg-white bg-contain bg-center bg-no-repeat" style={{ backgroundImage: `url("${drawing}")` }} /> : <div className="flex aspect-[21/10] items-center justify-center border border-dashed border-slate-300 bg-white text-sm text-black">Blank canvas</div>}</div>; })}</div></section>;
}
...
export function EyeExamModal({ open, value, onClose, onSave, onDraftChange, onContinue, inline = false, sidebar, activePage, onActivePageChange }: Props) {
  ...
  const footer = (
    <OptometryActionFooter inline={inline} pageLabel={`Page ${page + 1} of ${PAGES.length}`}>
      {!inline && !onContinue ? <button type="button" disabled={isSaving} onClick={onClose} className="rounded-xl border border-[#bfd7e8] bg-white px-4 py-2.5 text-sm font-medium text-black transition hover:bg-[#f3f8fb] disabled:opacity-100">
        Cancel
      </button> : null}
      <button type="button" disabled={page === 0 || isSaving} onClick={() => movePage(-1)} className="rounded-xl border border-[#bfd7e8] bg-white px-4 py-2.5 text-sm font-medium text-black transition hover:bg-[#f3f8fb] disabled:cursor-not-allowed disabled:opacity-100">
        Back
      </button>
      <button type="button" disabled={page === PAGES.length - 1 || isSaving} onClick={() => movePage(1)} className="rounded-xl bg-slate-700 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-slate-600 disabled:cursor-not-allowed disabled:opacity-100">
        Next
      </button>
      <button type="button" disabled={isSaving} onClick={async () => { if (await saveExam()) { if (!onContinue) onClose(); } }} className="rounded-xl border border-slate-900 bg-white px-4 py-2.5 text-sm font-semibold text-black transition hover:bg-slate-50 disabled:opacity-100">
        {isSaving ? "Saving..." : "Save"}
      </button>
      {onContinue ? <button type="button" disabled={isSaving} onClick={continueToConsultation} className="rounded-xl bg-[#2f8fd3] px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-[#287fc0] disabled:opacity-100">
