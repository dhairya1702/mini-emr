...
- Apply the current database schema from [db/schema.sql](/Users/dhairyalalwani/PycharmProjects/mr/db/schema.sql).
...
- Confirm Cloud SQL backups are enabled and that you know the restore path.
- Confirm the backend service account can read and write the patient attachment GCS bucket.
...
- Re-apply [db/schema.sql](/Users/dhairyalalwani/PycharmProjects/mr/db/schema.sql) if schema drift is suspected.
...
- If a production DB fix is needed, update [db/schema.sql](/Users/dhairyalalwani/PycharmProjects/mr/db/schema.sql) immediately so the repo stays authoritative.
