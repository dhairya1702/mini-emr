...
create table if not exists public.dashboard_revisions (
  ...
  billing_patients_revision bigint not null default 0,
  billing_invoices_revision bigint not null default 0,
...
create table if not exists public.notes (
  ...
  optometry_history jsonb not null default '{}'::jsonb,
  snapshot_optometry_history jsonb,
...
begin
  ...
  if not exists (select 1 from pg_constraint where conname = 'referral_packages_org_created_by_fk') then
    alter table public.referral_packages add constraint referral_packages_org_created_by_fk foreign key (org_id, created_by) references public.clinic_users(org_id, id);
  end if;
...
create trigger dashboard_patients_revision after insert or update or delete on public.patients
...
create trigger dashboard_check_ins_revision after insert or update or delete on public.public_check_in_requests
...
create trigger dashboard_patient_visits_queue_revision after insert or update or delete on public.patient_visits
...
create trigger dashboard_appointments_queue_revision after insert or update or delete on public.appointments
for each row execute function public.bump_dashboard_queue_revision();
drop trigger if exists dashboard_invoices_queue_revision on public.invoices;
create trigger dashboard_invoices_queue_revision after insert or update or delete on public.invoices
...
create trigger dashboard_invoice_items_queue_revision after insert or update or delete on public.invoice_items
for each row execute function public.bump_dashboard_invoice_revisions();
drop trigger if exists dashboard_notes_queue_revision on public.notes;
create trigger dashboard_notes_queue_revision after insert or update or delete on public.notes
for each row execute function public.bump_dashboard_queue_revision();
drop trigger if exists dashboard_catalog_items_queue_revision on public.catalog_items;
create trigger dashboard_catalog_items_queue_revision after insert or update or delete on public.catalog_items
for each row execute function public.bump_dashboard_queue_revision();
